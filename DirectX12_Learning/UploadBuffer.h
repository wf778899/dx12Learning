...
//! ==========================================================   Upload Buffer   ==========================================================
/*
   На этапе конструирования создаёт ресурс буфера в GPU с возможностью записи в него данных от  CPU  (D3D12_HEAP_TYPE_UPLOAD).   Количество
элементов задаётся в конструкторе, тип - в шаблоне. Если создаётся ресурс буфера констант,  то  размеры  его  элементов  пересчитываются до
кратности 256 байт (это требование харда по размещению константных буферов в памяти GPU). После создания  мапит  указатель  на этот ресурс.
Через него, с помощью  CopyData() можно читать/писать данные в буфер.																     */
